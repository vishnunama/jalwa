...
const listWithdrawalRequests = async (req, res) => {
  ...
  try {
    ...
    let withdraw
    if(req.query.type == "rsPay"){
       withdraw = await withdrawDB.getWithdrawalListRsPay({
        status: WITHDRAWAL_STATUS_MAP.PENDING,
      });
    }else{
       withdraw = await withdrawDB.getWithdrawalList({
        status: WITHDRAWAL_STATUS_MAP.PENDING,
      });
    }
...
const withdrawDB = {
  ...
  async getWithdrawalList({ userPhoneNumber, status }) {
    ...
    status === undefined
      ? await connection.query(
          "SELECT * FROM withdraw WHERE `phone` = ? ORDER BY `time` DESC",
          [userPhoneNumber]
        )
      : userPhoneNumber
        ? await connection.query(
            "SELECT * FROM withdraw WHERE `phone` = ? AND `status` = ? ORDER BY `time` DESC",
  ...
  async getWithdrawalListRsPay({ userPhoneNumber, status }) {
    let [withdrawalList] =
    status === undefined
      ? await connection.query(
          "SELECT * FROM withdraw WHERE `phone` = ? ORDER BY `time` DESC",
