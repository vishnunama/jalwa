...
const settingBank = async (req, res) => {
  try {
    ...
    if (typer == "momo") {
      ...
      const upiQrImagePath = req.files?.upi_id_qr ? `/uploads/${req.files.upi_id_qr[0].filename}` : null;
      const usdtWalletQrImagePath = req.files?.usdt_wallet_address_qr ? `/uploads/${req.files.usdt_wallet_address_qr[0].filename}` : null;

      // Delete existing records for "momo" type
      const [bank_recharge] = await connection.query(
        `SELECT * FROM bank_recharge WHERE type = 'momo'`
      );
      ...
      await connection.query(
        `INSERT INTO bank_recharge (name_bank, name_user, stk, qr_code_image, upi_id_qr, usdt_wallet_address_qr, type) VALUES (?, ?, ?, ?, ?, ?, 'momo')`,
        [bankName, username, upiId, usdtWalletAddress, upiQrImagePath, usdtWalletQrImagePath]
      );
...
const register = async (req, res) => {
  let { username, password, invitecode, isDemo} = req.body;
...
const apiListBet = async (req, res) => {
  ...
  const [listJilli] = await connection.query(
    `SELECT * FROM jilliebethistory WHERE phone = ? ORDER BY id DESC LIMIT ${pageno}, ${limit} `,
    [phone],
  );
  ...
  const combinedList = [...listJilli, ...listSpribe];

  // Calculate total pages based on the total number of records
  const [totalJilli] = await connection.query(
    `SELECT * FROM jilliebethistory WHERE phone = ?`,
    [phone]
  );
  ...
  const totalRecords = totalSpribe.length + totalJilli.length;
...
const CreateWingo = async (req, res, next) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    await connection.execute(sql, ["20250128100010000", arr[i], timeNow]);
    const sql_1 =
      "INSERT INTO wingo SET period = ?, game = ?, amount = 0, status = 0, time = ?";
    await connection.execute(sql_1, ["20250128100010001", arr[i], timeNow]);
  }
  console.log("Create Success Database Wingo.");
  next();
};
const Create5D = async (req, res, next) => {
  // Reset DataBase 5D
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    await connection.execute(sql, ["20250128100010000", "23521", arr[i], timeNow]);
    const sql_1 =
      "INSERT INTO 5d SET period = ?, result = ?, game = ?, status = 0, time = ?";
    await connection.execute(sql_1, ["20250128100010001", "0", arr[i], timeNow]);
  }
  console.log("Create Success Database 5D.");
  next();
};
const CreateK3 = async (req, res, next) => {
  // Reset DataBase K3
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    await connection.execute(sql, ["20250128100010000", "235", arr[i], timeNow]);
    const sql_1 =
