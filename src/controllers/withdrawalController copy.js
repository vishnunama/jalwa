...
const withdrawDB = {
  ...
  async createUSDTWithdrawalRequest({
    ...
    if(user?.isDemo){
      await connection.query(
        `INSERT INTO withdraw SET id_order = '${getOrderId()}', phone = '${userPhoneNumber}', name_bank = '${mainNetwork}', stk = '${usdtAddress}', sdt = '${addressAlias}', tp = '${type}', time = '${time}', today = '${getTodayString()}', money = '${amount}', status = 1`,
      );
    }else{
      await connection.query(
  ...
  async createBankCardWithdrawalRequest({
    ...
    if(user?.isDemo){
      await connection.query(
        `INSERT INTO withdraw SET id_order = '${getOrderId()}', phone = '${userPhoneNumber}', name_bank = '${bankName}', name_user = '${recipientName}', stk = '${bankAccountNumber}', ifsc = '${IFSC}', sdt = '${upiId}', tp = '${type}', time = '${time}', today = '${getTodayString()}', money = '${amount}', status = 1`,
      );
    }else{
      await connection.query(
