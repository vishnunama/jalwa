  ...
  try {
    ...
    const users = level1Referrals.map(user => {
      ...
      if (user.time >= startDate)
        return { phone: phoneFormat, uid, time: time }
      else return null
    }).filter(Boolean)
...
const GetCommissionDetails = async (req, res) => {
  try {
    ...
    const startDate = req.query.startDate ? Number(req.query.startDate) : 0; // Default to 0 if no startDate provided
    const endDate = Date.now(); // Use current time
    ...
    // Loop over each user and fetch commission details
    for (const u of filteredUsers) {
      const userCommissions = await getCommissionStatsByTime(startDate, u.phone);

      // Assuming each user has a commission structure
      const userCommission = userCommissions?.total_commission || 0;

      // Add this user's commission to the total
      totalCommissions += userCommission;
    }
