  ...
  try {
    ...
    const [JilliApiGame] = await connection.query(
      `SELECT 
          ...
          SUM(winAmount) AS jilliTotalWinAmount
       FROM 
          jilliebethistory 
       WHERE 
          phone = ? 
    ...
    const jilliBetCount = JilliApiGame[0]?.jilliBetCount || 0;
    const jilliTotalBetAmount = JilliApiGame[0]?.jilliTotalBetAmount || 0.0;
    const jilliTotalWinAmount = JilliApiGame[0]?.jilliTotalWinAmount || 0.0;
    ...
    return res.status(200).send({
      status: 200,
      totalBetAmount: totalBetAmount + parseFloat(jilliTotalBetAmount),
      list: [
        {
          title: "lottery",
        ...
        {
          title: "Jilli",
          totalBetAmount: parseFloat(jilliTotalBetAmount),
          numberOfBets: parseFloat(jilliBetCount),
          totalWinAmount: parseFloat(jilliTotalWinAmount),
        },
      ],
